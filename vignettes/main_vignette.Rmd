...
See Appendix A for a description of how indexing by choice set and choice object works.
...
Choice sets, subsets of the choice universe $\{x,y,z\}$, can be indexed using the strings `"x"`, `"y"`, `"xy"`, ..., `"xyz"` or by the set indices described in Appendix A.
...
```{r counts_RDS_2011}
n_objects <- 5
doubletons <- u_const$doubletons[1:u_const$n_doubletons[n_objects]]
print(RDS_2011_counts['Cash 1', n_objects, doubletons, ], na.print='-')
```
For this particular domain and subject, all choice frequencies are consistent with weak stochastic transitivity: from each binary choice set, the subject chose the lower alphabetically ranked object much more often than the higher.
...
The name of each object consists of the name of the experiment and one of the suffixes `_trials`, `_counts`, `_demographics` or `_raw`.
Take for example the population choice experiment described in McCausland, Marley and Davis-Stober and named `MMS_2019`.
...
The variable `set_index` is the menu index, a binary representation of the choice set, as described above.
...
```{r trials}
head(MMS_2019_trials[c('domain', 'subject', 'trial', 'set', 'choice', 'set_perm',
                       'set_index', 'choice_int')], 5)
```
...
```{r RP}
MMS_2019_trials %>% dplyr::select(set, choice, matches("^[a-e]{2}$")) %>% head(5)
```
...
```{r RP_plus}
MMS_2019_trials %>% filter(domain=="Beer") %>%
  dplyr::select(matches("^[a-e]{2}$")) %>% colSums
```
...
In the following code, we compute for every domain and every subset of size two or more, the p-value for the Pearson two-sample chi-squared test of the hypothesis that the counts in the two waves are drawn from the same multinomial distribution, then display a histogram of the 176 p-values.
```{r compare_waves}
...
Many different tests of the null hypothesis are possible; for the purposes of illustration we consider the results of the chi-squared test for each case.
The figure gives a histogram of the realized p-values.
...
The function `P_frequencies` computes choice frequencies from choice counts.
The following code computes and displays the choice frequencies that correspond to the cross section `N_bce` of counts constructed above.
These choice frequencies are an example of a RCS for the choice universe $\{b,c,e\}$.
```{r write_RCS}
P_bce <- P_frequencies(N_bce)  # Compute frequencies from count data
print(P_bce, na.print='-')   # and display.
```
Note that we are using the convention that for singleton sets, the choice frequencies for the only element of the set are equal to 1, even if the set is never presented.
...
The Wikipedia page on [ternary plots][terW] gives a nice overview of plotting ternary probabilities or frequencies in the regular 2-simplex, an equililateral triangle.
...
The blue triangle (pointing down) shows the region of ternary probabilities that is consistent with both regularity and the binary choice frequencies given by the observed counts in `N_bce`: those binary choice frequencies are displayed, as before, as hollow dots on the three sides of the triangle bce.
...
The red triangle (pointing up) shows the region of ternary probabilities consistent with the multiplicative inequality and the same binary choice frequencies.
```{r regularity}
triplot(label=c('b', 'c', 'e')) # Set up ternary plot, with labels and grid
plot_P3(P_bce)                  # Plot points 
...
Three context effects pertaining to stochastic discrete choice have attracted a lot of attention in Psychology, Economics, Marketing and other fields: the similarity effect, the compromise effect and the asymmetric dominance effect.
See @StobMarlMcCaTurn23 for details.
...
We can also check specific RCDs, arising from direct specification, simulation, or observed choice frequencies, to see if they exhibit context effects.
...
plot_P3(P_bce)                           # Plot frequencies from data
```
...
Most of the operations available in the `RanCh` package are relative to a universe $T$ with $n$ elements.
We need to be able to iterate through the $n$ elements of $T$, the $2^n-1$ non-empty subsets of $T$, and the $n!$ linear orders on $T$; the elements of $T$ are  interpreted as choice objects, the non-empty subsets of $T$ as menus and the linear orders on $T$ as preferences.
R objects created by the `RanCh` package often have dimensions indexed by object, menu or preference.
This appendix establishes consistent orderings for all three.

To define these orderings, we will denote any universe with $n$ elements as $T_n \equiv \{1,2,\ldots,n\}$, without substantive loss of generality.
However, the user may prefer to use symbols to represent objects, however.
In a running example, the user has in mind a universe $T \equiv \{a,b,c\}$.
The user can assign the names "a", "b", and "c" to the objects 1, 2 and 3, respectively.
Various functions in RanCh then assign names to each menu or preferences based on the object names supplied by the user.
The name of the menu $\{a,b\}$, for example, is `ab` and the name of the preference $b \succ c \succ a$ is `bca`.
In all this, we make use of the dimension naming system in R.
The user can refer to choice objects, menus and preferences by integer or name, interchangeably.

The following example illustrates how this works.
The first command of the following code creates a Luce random choice model for three objects.
The input, a *named vector*, gives the Luce weights $2$, $3$ and $5$ and establishes the object names `a`, `b` and `c`.
The second line displays the random choice model.
Rows are labelled by menu names and columns are labelled by object names.
The third and fourth commands both display $P_\{a,b\}(b)$, the probability of choosing $b$ from the menu $\{a,b\}$; menu and object are first indexed by number and then by name.
```{r P3}
P <- P_Luce(c(a=2,b=3,c=5))
print(P, na.print='-')
print(P[3,2])
print(P["ab", "b"])
```

Each menu, a non-empty subset $A \subseteq T$ is associated with an index from $1$ to $2^n-1$.
Each digit in the binary representation of the index of a menu $A$ serves as an inclusion indicator: for any $i \in \{1,\ldots,n\}$, $i \in A$ if and only if the $i$'th binary digit (or bit) of the index, counting from the right, is 1.
Thus, the singleton set with object $i$ has index $2^{i-1}$ and the index of any non-empty $A \subseteq T$ is $\sum_{i \in A} 2^{i-1}$.
This convention for indexing sets has some important computational advantages.
The bitwise “or” (`bitwOr` in R) of the indices of two sets gives the menu index of their union; the bitwise “and” (`bitwAnd`), the menu index of their intersection.

Preference orders are indexed by integers running from $1$ to $n!$ in increasing lexicographic order.
Thus for $T_3 = \{a, b, c\}$, the six preferences are, in sequence, $a \succ b \succ c$, $a \succ c \succ b$, $b \succ a \succ c$, $b \succ c \succ a$, $c \succ a \succ b$ and $c \succ b \succ a$.
The names used to represent these orders are `abc`, `acb`, `bac`, `bca`, `cab` and `cba`.

In some cases, we want to iterate through all pairs $(A,x)$ where $x \in A \subseteq T$ and $|T| \geq 2$.
We can do this using a single numeric index, running from 1 to $\sum_{i=2}^n \binom{n}{i} i = n (2^{n-1}-1)$.
We just need to establish a conventional order.
We order the $(A,x)$ lexicographically by increasing set size $|A|$ then by increasing menu index $A$ (within each set size) then by object index $x$.
Thus for the universe $T = \{a, b, c\}$, the order is
$(\{a,b\}, a)$, $(\{a,b\}, b)$, $(\{a,c\}, a)$, $(\{a,c\}, c)$, $(\{b,c\}, b)$, $(\{b,c\}, c)$, $(\{a,b,c\}, a)$, $(\{a,b,c\}, b)$, $(\{a,b,c\}, c)$.
The names used to represent these orders are `ab;a`, `ab;b`, `ac;a`, `ac;c`, `bc;b`, `bc:c`, `abc;a`, `abc;b`, and `abc;c`.
The point of ordering first by set size is computational efficiency in the analysis of experimental data where only binary choices, or only binary and ternary choices are observed.

The following example illustrates this flat indexing.
The first command creates a universe of three objects named `a`, `b` and `c`.
The second command creates a flat vector containing the non-redundant information in the Luce random choice model $P$.
The third command displays this named vector; element names indicate the menu and object names, separated by a semicolon.
```{r vectorize}
u <- create_universe(3, c("a", "b", "c"))
P_Ax <- vectorize(u, P)
print(P_Ax)
```

# Appendix B: Universe constants

The RanCh package provides a named list `u_const` of various constants pertaining to universes up to size $n_{\max} = 6$.

The following vectors, each of length $n_{\max}$, give dimension information for universes up to size $n_{\max}$.
For a given universe $T$ of cardinality $n$,

- `u_const$n_subsets[n]` is the number of non-empty subsets of the universe, equal to $2^n-1$;
- `u_const$n_orders[n]` is the number of strict preference orders over the elements of the universe, equal to $n!$;
- `u_const$n_singletons[n]` is the number of singleton subsets of $T$, equal to $n$;
- `u_const$n_doubletons[n]` is the number of doubleton subsets of $T$, equal to $\binom{n}{2}$;
- `u_const$n_tripletons[n]` is the number of tripleton subsets of $T$, equal to $\binom{n}{3}$;
- `u_const$n_Ax[n]` is the number of flat `Ax` indices, equal to $n(2^{n-1}-1)$

`u_const$subset_card` is a vector of length $2^{n_{\max}}-1$.
`u_const$subset_vectors` is a list of length $2^{n_{\max}}-1$.
If the variable `A` is the index of a choice set, then `u_const$subset_card[A]` is the number of elements in that choice set and `u_const$subset_vectors[[A]]` is a vector of length `u_const$subset_card[A]` giving the elements of choice set $A$.

`u_const$singletons`, `u_const$doubletons` and `u_const$tripletons` are vectors whose elements are the set indices of all the singleton, doubleton and tripleton subsets of a universe of $n_{\max}$ elements.
In all cases, the set indices are in order; thus, for a universe $T$ of size $n \leq n_{\max}$, `u_const$singletons[1:u_const$n_singletons[n]]` are the set indices of all the singleton subsets of a universe of $|T|$ elements, and similarly for doubletons and tripletons.

`u_const$member_table` is a $(2^{n_{\max}}-1) \times n_{\max}$ array of set membership indicators.
`u_const$member_table[A][x]` is 1 if $x \in A$ and `NA` otherwise.
