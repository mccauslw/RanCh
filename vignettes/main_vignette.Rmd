...
See @McCaMarl12 for some justification for this choice.
...
In the following code, we compute for every domain and every subset of size two or more, the p-value for the Pearson chi-squared test that counts in the two waves are from the same multinomial distribution.
```{r compare.waves}
...
Under the null hypotheses that the distributions are the same, the set of 176 p-values should be independent and uniformly distributed on $[0,1]$.
...
Five rejections out of 176 is less than the 8.8 we would expect under the null, on average.
