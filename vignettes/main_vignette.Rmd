...
```{r trials}
head(PC_trials[c('subj', 'domain', 'trial', 'subs', 'choice', 'subs_conf', 'subs_bin', 'choice_int')], 5)
```
...
The following table repeats the first five observations of the variables `subs` and `choice`, and includes the first five observations of the revealed preference indicators.
```{r RP}
head(PC_trials[c(c(4, 5), 9:18)])
```
...
```{r RP_plus}
colSums(PC_trials[PC_trials$domain=='Beer', 9:18])
```
