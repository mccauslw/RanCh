...
We need to be able to iterate through the $n$ elements of $T$, the $2^n-1$ non-empty subsets of $T$, and the $n!$ linear orders on $T$, interpreted as choice objects, menus and preferences, respectively.
R objects created by the `RanCh` package often have dimensions indexed by object, menu or preference.
This appendix establishes consistent orderings for objects, menus and preferences.

We make use of the dimension naming system in R to allow indexing by integers or names, interchangeably.
There will be times, usually for computational convenience, when we wish to treat the universe as a set of integers, as in  $T=\{1,2,\ldots,n\}$, and times when we use variable names, as in $T=\{a,b,\ldots\}$.
As a running example, we consider the universe $T_3 = \{a,b,c\}$, sometimes expressed as $T_3 = \{1,2,3\}$.

To create a universe structure for $T_3$, we will provide the names "a", "b" and "c" for objects 1, 2 and 3, respectively, as follows:
```{r u_define}
u <- create_universe(3, object_names = c("a", "b", "c"))
```
For this function, we can provide names or accept the default names "1", "2", etc.
Many `RanCh` functions that create structures set the dimension names of outputs to be consistent with the dimension names of inputs.

Each menu, a non-empty subset $A \subseteq T$ is associated with a *set index* from $1$ to $2^n-1$.
Each digit in the binary representation of the set index of a set $A$ serves as an inclusion indicator: for any $i \in \{1,\ldots,n\}$, $i \in A$ if and only if the $i$'th binary digit (or bit) of the set index, counting from the right, is 1.
Thus, the singleton set with object $i$ has set index $2^{i-1}$ and the set index of any non-empty $A \subseteq T$ is $\sum_{i \in A} 2^{i-1}$.
This convention for indexing sets has some important computational advantages.
The bitwise “or” (`bitwOr` in R) of the indices of two sets gives the set index of their union; the bitwise “and” (`bitwAnd`), the set index of their intersection.
...
In some cases, we replace the double index $(A,x)$, where $A$ is the index of a choice set and $x$ is the index of an object in $A$, with a flat index.
All pairs $(A,x)$ where $x \in A$ and where $A$ has at least two elements have an index, and only those pairs.
Thus the flat index runs from 1 to $\sum_{i=2}^n \binom{n}{i} i = n (2^{n-1}-1)$.
Pairs $(A,x)$ are ordered lexicographically by increasing set size $|A|$ then by increasing set index $A$ (within each set size) then by object index $x$.
