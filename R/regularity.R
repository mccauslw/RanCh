...
#'
#' \code{regularity} returns \code{TRUE} if the random choice structure \code{P} satisfies
#' regularity and \code{FALSE} otherwise.
...
regularity = function(P) {
  ...
  n_sets <- 2^n-1 # Number of subsets of {1,...,n}, excluding empty set
  for (A in 1:n_sets) { # Run through all non-empty subsets A of {1,...,n}
